...
_nllb_1_3b_model = None
_nllb_1_3b_tokenizer = None
_nllb_1_3b_device = None

_marian_zh_vi_model = None
_marian_zh_vi_tokenizer = None
...
def _translate_result(translated: list, **fields) -> dict:
    from services.subtitle_segmenter import apply_translate_line_wrap

    return {"translated": apply_translate_line_wrap(list(translated)), **fields}
...
def _unload_nllb_1_3b():
    """Release NLLB-1.3B VRAM so TTS model can reload."""
    global _nllb_1_3b_model, _nllb_1_3b_tokenizer
    import gc
    _nllb_1_3b_model = None
    _nllb_1_3b_tokenizer = None
    gc.collect()
...
def _unload_marian_zh_vi():
    """Release MarianMT Zh→Vi model from memory."""
    global _marian_zh_vi_model, _marian_zh_vi_tokenizer
    import gc
    _marian_zh_vi_model = None
    _marian_zh_vi_tokenizer = None
    gc.collect()
...
async def dub_translate(req: TranslateRequest):
    try:
        ...
        # NLLB-200 1.3B — Higher quality, especially for Asian language pairs (Zh↔Vi)
        if provider == "nllb_1_3b":
            flores_tgt = FLORES_CODES.get(req.target_lang, "eng_Latn")
            flores_src = FLORES_CODES.get(src_lang, "eng_Latn")

            def _translate_nllb_1_3b():
                global _nllb_1_3b_model, _nllb_1_3b_tokenizer, _nllb_1_3b_device
                import torch
                from transformers import AutoTokenizer, AutoModelForSeq2SeqLM
                ...
                try:
                    if _nllb_1_3b_tokenizer is None:
                        _nllb_1_3b_tokenizer = AutoTokenizer.from_pretrained("facebook/nllb-200-distilled-1.3B")
                    if _nllb_1_3b_model is None:
                        _nllb_1_3b_model = AutoModelForSeq2SeqLM.from_pretrained("facebook/nllb-200-distilled-1.3B")
                        if target_device != "cpu":
                            try:
                                _nllb_1_3b_model = _nllb_1_3b_model.to(target_device)
                                _nllb_1_3b_device = target_device
                            except Exception as e:
                                logger.warning("NLLB-1.3B %s placement failed, falling back to CPU: %s", target_device, e)
                                _nllb_1_3b_device = "cpu"
                        else:
                            _nllb_1_3b_device = "cpu"
                except Exception as e:
                    logger.exception("NLLB-1.3B model load failed")
                    return [{"id": seg.id, "text": seg.text, "error": f"Model load error: {str(e)}"} for seg in req.segments]
                ...
                for seg in req.segments:
                    try:
                        ...
                        _nllb_1_3b_tokenizer.src_lang = flores_src
                        inputs = _nllb_1_3b_tokenizer(seg.text, return_tensors="pt")
                        if _nllb_1_3b_device and _nllb_1_3b_device != "cpu":
                            inputs = {k: v.to(_nllb_1_3b_device) for k, v in inputs.items()}

                        forced_bos_token_id = _nllb_1_3b_tokenizer.convert_tokens_to_ids(tgt)
                        try:
                            translated_tokens = _nllb_1_3b_model.generate(
                                **inputs, forced_bos_token_id=forced_bos_token_id, max_length=400
                            )
                        except (RuntimeError, NotImplementedError) as e:
                            if _nllb_1_3b_device == "mps":
                                logger.warning("MPS generate failed for NLLB-1.3B, retrying on CPU: %s", e)
                                _nllb_1_3b_model.to("cpu")
                                _nllb_1_3b_device = "cpu"
                                inputs = {k: v.to("cpu") for k, v in inputs.items()}
                                translated_tokens = _nllb_1_3b_model.generate(
                                    **inputs, forced_bos_token_id=forced_bos_token_id, max_length=400
                                ...
                                raise
                        translated_text = _nllb_1_3b_tokenizer.batch_decode(translated_tokens, skip_special_tokens=True)[0]
                        results.append({"id": seg.id, "text": translated_text})
                    except Exception as e:
                        results.append({"id": seg.id, "text": seg.text, "error": str(e)})
                return results

            translated = await loop.run_in_executor(_gpu_pool, _translate_nllb_1_3b)
            if os.environ.get("OMNIVOICE_UNLOAD_NLLB", "1") == "1":
                _unload_nllb_1_3b()
            return _translate_result(
                translated,
        ...
        # MarianMT Helsinki-NLP opus-mt-zh-vi — Specialized Chinese→Vietnamese
        # Lightweight (~300MB), faster than NLLB, trained specifically on Zh↔Vi pairs.
        # Only suitable for zh→vi direction. Falls back gracefully for other src/tgt pairs.
        if provider == "marian_zh_vi":
            MARIAN_ZH_VI_MODEL = "Helsinki-NLP/opus-mt-zh-vi"

            def _translate_marian_zh_vi():
                global _marian_zh_vi_model, _marian_zh_vi_tokenizer
                from transformers import MarianMTModel, MarianTokenizer

                try:
                    if _marian_zh_vi_tokenizer is None:
                        _marian_zh_vi_tokenizer = MarianTokenizer.from_pretrained(MARIAN_ZH_VI_MODEL)
                    if _marian_zh_vi_model is None:
                        _marian_zh_vi_model = MarianMTModel.from_pretrained(MARIAN_ZH_VI_MODEL)
                except Exception as e:
                    logger.exception("MarianMT zh-vi model load failed")
                    return [{"id": seg.id, "text": seg.text, "error": f"Model load error: {str(e)}"} for seg in req.segments]
                ...
                for seg in req.segments:
                    try:
                        if not seg.text or not seg.text.strip():
                            results.append({"id": seg.id, "text": seg.text})
                            continue
                        # MarianMT is a seq2seq model — tokenize and generate
                        inputs = _marian_zh_vi_tokenizer(
                            [seg.text], return_tensors="pt", padding=True, truncation=True, max_length=512
                        )
                        translated_tokens = _marian_zh_vi_model.generate(**inputs)
                        translated_text = _marian_zh_vi_tokenizer.batch_decode(
            ...
            translated = await loop.run_in_executor(_gpu_pool, _translate_marian_zh_vi)
            if os.environ.get("OMNIVOICE_UNLOAD_NLLB", "1") == "1":
                _unload_marian_zh_vi()
            return _translate_result(
                translated,
                target_lang=req.target_lang,
                source_lang=src_lang,
...
async def _maybe_cinematic(translated, req, src_lang, loop):
    ...
    apply_translate_line_wrap(translated)
    ...
    apply_translate_line_wrap(merged)
