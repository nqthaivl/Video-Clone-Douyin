...
def _repo_has_complete_weights(repo_id: str) -> bool:
    """Only accept snapshots that contain an actual, complete weight file."""
    if repo_id == "pyannote/speaker-diarization-3.1":
        # Diarization model chỉ cần config.yaml tồn tại trong snapshot
        ...
        for root in _hub_cache_roots():
            ...
            try:
                for revision in os.listdir(snapshots):
                    ...
                    if not os.path.isdir(revision_dir):
                        continue
                    config_file = os.path.join(revision_dir, "config.yaml")
            ...
            for revision in os.listdir(snapshots):
                ...
                for dirpath, _dirs, files in os.walk(revision_dir, followlinks=True):
                    for filename in files:
                        if filename.endswith(".incomplete"):
                            continue
                        floor = _WEIGHT_FLOORS.get(os.path.splitext(filename)[1].lower())
                        if floor is None:
                            continue
                        try:
                            if os.path.getsize(os.path.join(dirpath, filename)) >= floor:
                                return True
                        except OSError:
                            continue
        except OSError:
    ...
    try:
        ...
        for entry in info.repos:
            if entry.repo_id == repo_id and entry.size_on_disk > 0:
                return _repo_has_complete_weights(repo_id)
        return False
    except Exception as e:
...
def recommendations():
    ...
    if is_mac_arm:
        ...
        if has_cuda:
            ...
            rationale = (
                ...
                "ASR fallback. MLX / mlx-audio are Apple-Silicon-only and don't apply here."
            )
        else:
            rationale = (
    ...
    try:
        ...
        cached_ids = {
            entry.repo_id
            for entry in info.repos
            if entry.size_on_disk > 0 and _repo_has_complete_weights(entry.repo_id)
        }
    except Exception as e:
