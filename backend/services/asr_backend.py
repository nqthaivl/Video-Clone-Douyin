...
def _preferred_asr_repo() -> str | None:
    from core import prefs
    repo = prefs.get("asr_model_repo")
    if isinstance(repo, str) and repo.strip():
        return repo.strip()
    return None


def _whisper_size_from_repo(repo_id: str) -> str:
    low = repo_id.lower()
    if "large-v3" in low or "large_v3" in low:
        return "large-v3"
    if "large-v2" in low or "large_v2" in low:
        return "large-v2"
    if "/large" in low or low.endswith("-large"):
        return "large"
    if "medium" in low:
        return "medium"
    if "small" in low:
...
class WhisperXBackend(ASRBackend):
    ...
    def __init__(self):
        pref = _preferred_asr_repo()
        if pref and (
            "whisper" in pref.lower()
            or pref.startswith("Systran/")
            or pref.startswith("deepdml/")
        ):
            self._model_name = _whisper_size_from_repo(pref)
        else:
    ...
    def transcribe(self, audio_path: str, *, word_timestamps: bool = True, language: str | None = None) -> dict:
        ...
        self._ensure_asr()
        logger.info("whisperx transcribing %s (word_timestamps=%s, language=%s)", audio_path, word_timestamps, language)
        ...
        try:
            result = self._asr.transcribe(audio, language=language)
        except IndexError:
...
class FasterWhisperBackend(ASRBackend):
    ...
    def __init__(self):
        ...
        if pref and (
            pref.startswith("Systran/")
            or pref.startswith("deepdml/")
            or "whisper" in pref.lower()
        ):
    ...
    def transcribe(self, audio_path: str, *, word_timestamps: bool = True, language: str | None = None) -> dict:
        self._ensure_model()
        logger.info(
            "faster-whisper transcribing %s (word_timestamps=%s, language=%s)",
            audio_path, word_timestamps, language,
        )
...
class MLXWhisperBackend(ASRBackend):
    ...
    def __init__(self, model_name: str | None = None):
        pref = _preferred_asr_repo()
        if pref and pref.startswith("mlx-community/") and "whisper" in pref.lower():
            self._model_name = pref
        else:
    ...
    def transcribe(self, audio_path: str, *, word_timestamps: bool = True, language: str | None = None) -> dict:
        import mlx_whisper
        logger.info(
            "MLX Whisper transcribing %s (model=%s, word_timestamps=%s, language=%s)",
            audio_path, self._model_name, word_timestamps, language,
        )
...
class FunASRBackend(ASRBackend):
    ...
    def __init__(self):
        pref = _preferred_asr_repo()
        default = os.environ.get("ASR_MODEL_FUNASR", "iic/SenseVoiceSmall")
        if pref in ("iic/SenseVoiceSmall", "FunAudioLLM/SenseVoiceSmall"):
            self._model_name = pref
        elif pref and "sensevoice" in pref.lower():
            self._model_name = pref
        else:
            self._model_name = default
    ...
    def is_available(cls) -> tuple[bool, str]:
        ...
        try:
            from api.routers.setup.models import get_catalog_entry, model_is_installed
            pref = _preferred_asr_repo()
            repo = pref or "iic/SenseVoiceSmall"
            entry = get_catalog_entry(repo)
            if entry and model_is_installed(entry):
                return True, "ready"
            return True, "Tải FunASR SenseVoice tại Cài đặt → Quản lý Models"
        except Exception:
            return True, "ready"
...
def _auto_detect() -> str:
    ...
    Preference order:
      - Mac Apple Silicon: mlx-whisper (if available) -> whisperx -> faster-whisper
      - Windows / Linux: whisperx -> faster-whisper
    """
    from core.device_caps import mlx_supported
    is_mac_arm = False
    try:
...
def active_backend_id() -> str:
    ...
    # Prioritize whisperx as the default if available.
    # If the user has not explicitly set a preference, or if the preference is faster-whisper
    # (which was the old fallback/default) and whisperx is now available, prefer whisperx.
    ok_wx, _ = WhisperXBackend.is_available()
    if ok_wx and (not picked or picked == "faster-whisper"):
        return "whisperx"
