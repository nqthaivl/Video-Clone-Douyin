...
async def probe_video_dimensions(path: str) -> "tuple[int, int] | None":
    ...
    info = await probe_video_stream_info(path)
    if not info:
        return None
    return info["width"], info["height"]


def _normalize_rotation_degrees(rotation: int) -> int:
    rot = int(rotation or 0)
    if rot in (-90, 90, 180, 270, -180, -270):
        return rot
    rot = rot % 360
    return rot - 360 if rot > 180 else rot


def display_dimensions_from_stream(width: int, height: int, rotation: int = 0) -> tuple[int, int]:
    """Map stored stream size to display-oriented width/height."""
...
def rotation_transpose_filter(video_map: str, rotation: int) -> tuple[str, str] | None:
    """Return (filter_fragment, output_label) to orient frames for display."""
    rot = int(rotation or 0)
    if rot == 0:
        return None
    if video_map.startswith("["):
        src = video_map
    elif video_map.endswith(":v:0"):
        src = f"[{video_map[:-2]}]"
    elif video_map.endswith(":v"):
        src = f"[{video_map}]"
    else:
        src = f"[{video_map}:v]"
    # stream side_data rotation: counter-clockwise degrees for correct display.
    if rot in (-90, 270):
        op = "transpose=1"
    elif rot in (90, -270):
        op = "transpose=2"
    elif abs(rot) == 180:
        op = "transpose=2,transpose=2"
    else:
        return None
    return f"{src}{op}[vrot]", "[vrot]"


async def probe_video_stream_info(path: str) -> "dict | None":
    """Return width, height, rotation (degrees) for the first video stream."""
    ffprobe = find_ffprobe()
    if not ffprobe or not os.path.isfile(path):
        return None
    try:
        ...
        logger.debug("probe_video_stream_info failed for %s: %s", os.path.basename(str(path)), e)
        return None


async def probe_video_display_dimensions(path: str) -> "tuple[int, int] | None":
    """Return (display_width, display_height) accounting for rotation metadata."""
    info = await probe_video_stream_info(path)
    if not info:
        return None
    return display_dimensions_from_stream(info["width"], info["height"], info.get("rotation", 0))
