...
# Translated subtitles: max words per line when wrapping after dub translation.
TRANSLATE_MAX_WORDS_PER_LINE = 8
# Back-compat alias (translate-only; ASR no longer wraps).
ASR_MAX_WORDS_PER_LINE = TRANSLATE_MAX_WORDS_PER_LINE


def _word_units(text: str) -> list[str]:
    """Whitespace-separated words, or one unit per character when there are no spaces."""
...
def wrap_subtitle_text(text: str, max_words: int = TRANSLATE_MAX_WORDS_PER_LINE) -> str:
    """Wrap *text* so each line has at most *max_words* words (not characters)."""
    ...
    if "\n" in cleaned:
        return "\n".join(
            wrap_subtitle_text(part, max_words)
            for part in cleaned.split("\n")
            if part.strip()
        )

    units = _word_units(cleaned)
    if not units:
        return cleaned

    spaced = bool(re.search(r"\s", cleaned))
    lines: list[str] = []
    for offset in range(0, len(units), max_words):
        chunk = units[offset : offset + max_words]
        lines.append(" ".join(chunk) if spaced else "".join(chunk))
    return "\n".join(lines)


def apply_translate_line_wrap(
    items: list[dict],
    *,
    max_words: int = TRANSLATE_MAX_WORDS_PER_LINE,
) -> list[dict]:
    """Wrap translated ``text`` fields after dub translation (max words per line)."""
    for item in items or []:
        if not isinstance(item, dict) or item.get("error"):
            continue
        raw = str(item.get("text") or "")
        if raw.strip():
            item["text"] = wrap_subtitle_text(raw, max_words)
    return items
